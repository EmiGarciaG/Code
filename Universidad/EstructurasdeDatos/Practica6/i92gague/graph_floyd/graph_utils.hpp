...
std::shared_ptr<WGraph<T>> create_wgraph(std::istream &in) noexcept(false)
{
    ...
    for(size_t z=0; z<size; z++){
        in>>u>>v>>distancia_nodo;
        graph->set_weight(graph->find(u), graph->find(v), distancia_nodo);
        if(is_directed==false){
            graph->set_weight(graph->find(v), graph->find(u), distancia_nodo);
        }
    }
    return graph;
}
