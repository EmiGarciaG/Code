...
//		k3smoothing_p2 = 3. * ksmoothing_p2;
		...
		for (auto pair : memory) {
			...
			if (pair.second) {
				...
				if(iWon){
					if(first->numWins<MAXOCCURRENCES){
				 		first->numWins++;
				 	}
				 	else{
				 		int num=rand()%2;
				 		if(num==1 && numTies>MINOCCURRENCES){
				 			numTies--;
				 		}
				 		else if(num==0 && numLost>MINOCCURRENCES){
				 			numLost--;
				 		}
				 	}
				}
				else if(iLost){
				 	if(first->numLost<MAXOCCURRENCES){
				 		first->numLost++;
				 	}
				 	else{
				 		int num=rand()%2;
				 		...
				 		else if(num==0 && numWins>MINOCCURRENCES){
				 			numWins--;
				 		}
				 	}
				}
				else{
					if(first->numTies<MAXOCCURRENCES){
						first->numTies++;
					}
					else{
						int num=rand()%2;
				 		if(num==1 && numLost>MINOCCURRENCES){
				 			numLost--;
				 		}
				 		else if(num==0 && numWins>MINOCCURRENCES){
				 			numWins--;
