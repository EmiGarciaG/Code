...
int received = recvfrom(serverSocket, data, dataLength, 0, (struct
sockaddr *) &Client, &clientLength);
if (received){
printf("Request received: %s\n", data);
char * response = getTime(data, dataLength);
printf("Sending response: %s\n", response);
int sended = sendto(serverSocket, response, dataLength, 0, (struct
sockaddr *) &Client, clientLength);
free(response);
...
else if(!strcmp(format, "DAYTIME")) strftime(string,80,"%A, %d de %B de %Y; %H:%M:%S", localTime);
return string; 
}