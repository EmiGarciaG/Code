...
int main(){
   ...
   ----------------------------------------------------------------------*/
    ...
    /* -----------------------------------------------------------------
		...
		-------------------------------------------------------------------*/
      	int recibido=recvfrom(Socket_Servidor, &contador, sizeof(contador), 0, (struct sockaddr *) &Cliente, &Longitud_Cliente);
		/* -----------------------------------------------------------------
		...
		if(recibido>0){
      	...
      	/* ------------------------------------------------------------------
				...
				--------------------------------------------------------------------*/
			int enviado = sendto (Socket_Servidor, &contador, sizeof(contador), 0, (struct sockaddr *) &Cliente, Longitud_Cliente);
		}
    }
	 close(Socket_Servidor);
	return 0;
